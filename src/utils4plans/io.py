...
def handle_write_logic(p: Path, OVERWRITE=False):
    # TODO -> include append
    # parent = get_or_make_folder_path(p.parent)
    # TODO handle if parent folder != exist 
    if OVERWRITE:
        StyledConsole.print(f"Going to overwrite: {p} ", style="warning")
...
def write_pickle(item, folder_path: Path, file_stem: str, OVERWRITE=False):
    p = folder_path / make_pickle_name(file_stem)
...
def read_pickle(folder_path: Path, file_stem: str):
    with open(folder_path / f"{file_stem}.pickle", "rb") as handle:
        result = pickle.load(handle)
    return result


def write_json(item, folder_path: Path, file_stem: str, OVERWRITE=False):
    p = folder_path / make_json_name(file_stem)
...
def read_json(path_to_inputs: Path, file_stem):
    path = check_folder_exists_and_return(path_to_inputs / make_json_name(file_stem))
